...
# IMPORTANT: WEEE = 0, Food = 0, Garden = 0
# ============================================================
VALUE_FACTORS = pd.DataFrame([
    ...
    {"fraction":"WEEE",            "gbp_per_t_min": 0.0,   "gbp_per_t_mid": 0.0,   "gbp_per_t_max": 0.0,   "source":"Not modelled"},
...
with tabTS:
    ...
    st.markdown("### All treatment pathways over time (share of annual pathway mix)")
    route_mix = (
        long[long["route"].isin(TREATMENT_ROUTES)]
        .groupby(["year", "route"], as_index=False)
        .agg(tonnes=("tonnes", "sum"))
    )
    route_mix["pct"] = route_mix["tonnes"] / route_mix.groupby("year")["tonnes"].transform("sum")
    fig_route_mix = px.area(route_mix, x="year", y="pct", color="route", custom_data=["tonnes", "pct"])
    fig_route_mix = apply_paper_style(
        fig_route_mix,
        title="Treatment pathways over time in the observed system",
        x_title="Year",
        y_title="Share of treatment pathways (%)",
        tickangle=0
    )
    fig_route_mix.update_yaxes(tickformat=".0%", range=[0, 1])
    fig_route_mix.update_traces(
        hovertemplate=(
            "Year: %{x}<br>"
    ...
    st.plotly_chart(fig_route_mix, use_container_width=True)
    figure_download_buttons(fig_route_mix, "fig_treatment_pathways_over_time_pct")
...
with tabScen:
    ...
    fig2 = apply_paper_style(
        fig2,
        title="Changes in treatment pathway distribution across scenarios",
        x_title="Scenario",
        y_title="Share of treatment pathways (%)"
    )
    ...
    with c1:
        ...
        fig3 = apply_paper_style(
            fig3,
            title=f"Total greenhouse-gas emissions under baseline and scenario conditions ({unit_label})",
            x_title="Scenario",
            y_title=f"Emissions ({unit_label})"
        )
        emin = float(summ[f"Emissions_{unit_label}"].min())
        emax = float(summ[f"Emissions_{unit_label}"].max())
    ...
    scenario_tonnes = (
        d.groupby("scenario", as_index=False)
        .agg(TotalTonnes=("tonnes", "sum"))
    )

    summ_int = summ.merge(scenario_tonnes, on="scenario", how="left")
    summ_int["EmissionIntensity_kgCO2e_per_t"] = np.where(
        summ_int["TotalTonnes"] > 0,
        ems.set_index("scenario").loc[summ_int["scenario"], "kgCO2e"].to_numpy() / summ_int["TotalTonnes"],
        np.nan,
    )

    st.dataframe(
        summ_int[["scenario", "TotalTonnes", "EmissionIntensity_kgCO2e_per_t"]].round(2),
    ...
    fig_intensity = px.bar(
        ...
        custom_data=["TotalTonnes"],
    )
    fig_intensity = apply_paper_style(
        fig_intensity,
        title="Emission intensity by scenario",
        x_title="Scenario",
        y_title="kgCO2e per tonne of waste",
    )
    fig_intensity.update_traces(
        ...
        hovertemplate=(
            ...
            "Total tonnes: %{customdata[0]:,.0f}<extra></extra>"
        )
    )
...
with tabPriority:
    ...
    if len(adj) == 0:
        ...
        st.markdown("### Matrix input table (use this to verify WEEE = 0)")
        st.dataframe(
...
with tabSank:
    ...
    dd_em = scen_em[(scen_em["year"] == year) & (scen_em["route"].isin(TREATMENT_ROUTES))].copy()
    ...
    if metric.startswith("Emissions"):
        ...
        fig6 = sankey_scenario_fraction_route(
            ...
            title="Scenario–fraction–route links sized by absolute emissions (tCO2e)"
        )
    else:
        fig6 = sankey_scenario_fraction_route(
...
with tabCouncil:
    ...
    dd_em = scen_em[(scen_em["year"] == year) & (scen_em["Council Name"] == council) & (scen_em["scenario"] == scenario)].copy()
