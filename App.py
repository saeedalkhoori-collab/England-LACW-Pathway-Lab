...
def apply_paper_style(fig, title=None, x_title=None, y_title=None):
...
def figure_download_buttons(fig, base_filename):
    png_buf = io.BytesIO()
    fig.write_image(png_buf, format="png", width=1800, height=1000, scale=3)
    st.download_button(
        label="Download PNG",
        data=png_buf.getvalue(),
        file_name=f"{base_filename}.png",
        mime="image/png",
        key=f"{base_filename}_png"
    )

    pdf_buf = io.BytesIO()
    fig.write_image(pdf_buf, format="pdf", width=1800, height=1000)
    st.download_button(
        label="Download PDF",
        data=pdf_buf.getvalue(),
        file_name=f"{base_filename}.pdf",
        mime="application/pdf",
        key=f"{base_filename}_pdf"
    )

    svg_buf = io.BytesIO()
    fig.write_image(svg_buf, format="svg", width=1800, height=1000)
    st.download_button(
        label="Download SVG",
        data=svg_buf.getvalue(),
        file_name=f"{base_filename}.svg",
        mime="image/svg+xml",
        key=f"{base_filename}_svg"
    )
...
with tabTS:
    ...
    st.markdown("### All treatment pathways over time (tonnes)")
    route_mix = (
        long[long["route"].isin(TREATMENT_ROUTES)]
        .groupby(["year", "route"], as_index=False)
        .agg(tonnes=("tonnes", "sum"))
    )
    fig_route_mix = px.area(route_mix, x="year", y="tonnes", color="route")
    ...
    figure_download_buttons(fig_route_mix, "fig_treatment_pathways_over_time")
...
with tabScen:
    ...
    fig1 = px.bar(summ.sort_values("RecoveryRate"), x="scenario", y="RecoveryRate", text="RecoveryRate")
    ...
    fig1.update_traces(texttemplate="%{text:.1%}", textposition="outside", cliponaxis=False)
    ...
    fig2 = px.bar(mix_total, x="scenario", y="tonnes", color="route", barmode="stack")
    ...
    figure_download_buttons(fig2, "fig_route_mix_by_scenario")

    c1, c2 = st.columns(2)
    with c1:
        fig3 = px.bar(summ.sort_values(f"Emissions_{unit_label}"), x="scenario", y=f"Emissions_{unit_label}")
    ...
    with c2:
        fig4 = px.bar(summ.sort_values("RecoveredValue_GBP"), x="scenario", y="RecoveredValue_GBP")
