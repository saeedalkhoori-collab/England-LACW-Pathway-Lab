...
st.markdown("### Exact treatment pathway shares by year (%)")

route_pct_table = (
    route_mix
    .assign(pct=lambda d: d["pct"] * 100)
    .pivot(index="route", columns="year", values="pct")
    .round(2)
)

st.dataframe(route_pct_table)

csv = route_pct_table.to_csv().encode("utf-8")
st.download_button(
    "Download pathway shares by year (CSV)",
    csv,
    "pathway_shares_by_year.csv",
    "text/csv"
)
